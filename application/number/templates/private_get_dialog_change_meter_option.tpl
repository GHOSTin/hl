...
{% block dialog %}
	...
	<ul>
		<li>
			...
			<input type="text" class="dialog-input-serial" value="{{ old_meter.get_serial() }}">
		</li>
		<li>
			<span>Дата выпуска</span>
			<input type="text" class="dialog-input-date_release" value="{{ old_meter.get_date_release()|date('d.m.Y') }}">
		</li>
		<li>
			<span>Дата установки</span>
			<input type="text" class="dialog-input-date_install" value="{{ old_meter.get_date_install()|date('d.m.Y') }}">
		</li>
		<li>
			<span>Дата последней поверки</span>
			<input type="text" class="dialog-input-date_checking" value="{{ old_meter.get_date_checking()|date('d.m.Y') }}">
		</li>
		<li>
		...
		<li>
			...
			<textarea style="width:90%" class="dialog-textarea-comment">{{ old_meter.get_comment() }}</textarea>
		</li>
	</ul>
{% endblock dialog %}
{% block buttons %}
