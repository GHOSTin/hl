...
class model_number{
	...
	*/
	...
	/*
	* Изменяет комментария счтечика.
	*/
	public static function update_meter_comment(data_company $company,
												data_number2meter $meter,
												$comment){
		try{
			$meter->verify('number_id', 'meter_id', 'serial');
			...
			$old_meter = $meters[0];
			model_number2meter::is_data_number2meter($old_meter);
			$old_meter->comment = $comment;
			$old_meter->verify('number_id', 'meter_id', 'serial', 'comment');
			$sql = new sql();
			$sql->query("UPDATE `number2meter` SET `comment` = :comment
						WHERE `company_id` = :company_id AND `number_id` = :number_id
						AND `meter_id` = :meter_id AND `serial` = :serial");
			$sql->bind(':meter_id', $old_meter->meter_id, PDO::PARAM_INT);
			$sql->bind(':serial', $old_meter->serial, PDO::PARAM_STR);
			$sql->bind(':comment', $old_meter->comment, PDO::PARAM_STR);
			$sql->bind(':number_id', $old_meter->number_id, PDO::PARAM_INT);
			$sql->bind(':company_id', $company->id, PDO::PARAM_INT);
			$sql->execute('Проблема при обновление комментария счетчика.');
			$sql->commit();
		}catch(exception $e){
			$sql->rollback();
			if($e instanceof e_model)
	...
	*/
	...
	/*
	* Изменяет status счетчика.
	*/
	public static function update_meter_status(data_company $company,
											data_number2meter $meter, $status){
		try{
			$meter->verify('number_id', 'meter_id', 'serial');
			$sql = new sql();
			$sql->begin();
			...
			model_number2meter::is_data_number2meter($meter);
			$meter->status = $status;
			$meter->verify('number_id', 'meter_id', 'serial', 'status');
			$sql = new sql();
			$sql->query("UPDATE `number2meter` SET `status` = :status
						WHERE `company_id` = :company_id AND `number_id` = :number_id
						AND `meter_id` = :meter_id AND `serial` = :serial");
			$sql->bind(':meter_id', $meter->meter_id, PDO::PARAM_INT);
			$sql->bind(':serial', $meter->serial, PDO::PARAM_STR);
			...
			$sql->bind(':number_id', $meter->number_id, PDO::PARAM_INT);
			$sql->bind(':company_id', $company->id, PDO::PARAM_INT);
			$sql->execute('Проблема при обновление статуса поверки.');
			$sql->commit();
		}catch(exception $e){
			$sql->rollback();
			if($e instanceof e_model)
	...
	/*
	* Изменяет место установки счетчика.
	*/
	public static function update_meter_place(data_company $company,
											data_number2meter $meter, $place){
		try{
			$meter->verify('number_id', 'meter_id', 'serial');
			$sql = new sql();
			$sql->begin();
			...
			$meter->place = $place;
			$meter->verify('number_id', 'meter_id', 'serial', 'place');
			$sql = new sql();
			$sql->query("UPDATE `number2meter` SET `place` = :place
						WHERE `company_id` = :company_id AND `number_id` = :number_id
						AND `meter_id` = :meter_id AND `serial` = :serial");
			$sql->bind(':meter_id', $meter->meter_id, PDO::PARAM_INT);
			$sql->bind(':serial', $meter->serial, PDO::PARAM_STR);
			$sql->bind(':place', $meter->place, PDO::PARAM_STR);
			$sql->bind(':number_id', $meter->number_id, PDO::PARAM_INT);
			$sql->bind(':company_id', $company->id, PDO::PARAM_INT);
			$sql->execute('Проблема при обновление места установки счетчика.');
			$sql->commit();
		}catch(exception $e){
			$sql->rollback();
			if($e instanceof e_model)
