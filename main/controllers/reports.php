...
class reports{
  ...
  public function clear_filter_query(Application $app){
    ...
    return $app['twig']->render('report\clear_filter_query.tpl',
                                ['filters' => $filters]);
  }
  ...
  public function get_query_reports(Application $app){
    ...
    if(!empty($filters['street']))
      $houses = $app['em']->getRepository('data_house')
  ...
  public function report_query_one(Request $request, Application $app){
    set_time_limit(0);
    $model = $app['\main\models\report_query'];
    $queries = $model->get_queries();
    return $app['twig']->render('report\report_query_one.tpl',
                                ['queries' => $queries]);
  }
  ...
  public function report_query_one_xls(Request $request, Application $app){
    ...
    $model = $app['\main\models\report_query'];
    $queries = $model->get_queries();
    return $app['twig']->render('report\report_query_one_xls.tpl',
                                ['queries' => $queries]);
  }

  public function set_filter_query_department(Request $request,
                                              Application $app){
    $model = $app['\main\models\report_query'];
  ...
  public function set_filter_query_street(Request $request, Application $app){
    ...
    return $app['twig']->render('report\set_filter_query_street.tpl',
                                ['street' => $street]);
  }
