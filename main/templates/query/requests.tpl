...
<div class="ibox-content m-b">
  ...
  {% for request in number_requests %}
    <li class="request" time="{{ request.get_time() }}" number="{{ request.get_number().get_id() }}">
      <p>{{ request.get_number().get_address() }}</p>
      <span>{{ request.get_message() }}</span>
      <a class="get_dialog_create_query_from_request"> cоздать заявку</a> или <a class="get_dialog_abort_query_from_request">отвергнуть запрос</a>
    </li>
  {% endfor %}
  </ul>
  {% endif %}
