<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	...
	<head>
		...
		<title>Печатная форма заявки №{{ query.get_number() }} от {{ query.get_time_open()|date('H:i d.m.Y') }}</title>
		<style>
    ...
    <header>
      <nav class="navbar navbar-inverse navbar-fixed-top">
        <div class="navbar-inner">
          <a class="btn" onclick="window.print(); return false;"><i class="icon-print"></i>Печать</a>
          <a class="btn" onclick="window.close();"><i class="icon-remove"></i> Отмена</a>
        </div>
      </nav>
    </header>
	<div class="main">
	...
	<div class="main-block">
	{% if query.get_contact_fio() != false or query.get_contact_telephone() != false or query.get_contact_cellphone() != false%}
		<div>Заявку подал:
			{% if query.get_contact_fio() != false %}
				{{ query.get_contact_fio() }}
			{% endif %}
			{% if query.get_contact_telephone() != false %}
				тел. {{ query.get_contact_telephone() }}
			{% endif %}
			{% if query.get_contact_cellphone() != false %}
				сот. {{ query.get_contact_cellphone() }}
			{% endif %}
		</div>
	{% endif %}
		<table style="margin:2mm 0mm 0mm 0mm; width:194mm">
			<tr>
				<td style="border-bottom: 1px solid black; width:49%; height:8mm;">Сотрудники: диспетчер
					{{ query.get_creator().get_lastname() }} {{ query.get_creator().get_firstname() }} {{ query.get_creator().get_middlename() }}
				</td>
